...
def get_review_ready_words(user):
    ...
    Trả về tuple (cutoff_time, queryset) với:
      - cutoff_time = max(min(next_review) + 180 phút, now + 180 phút)
    ...
    if min_next_review is None:
        # Trong trường hợp không có từ nào, cutoff_time có thể là now + 180 phút
        cutoff_time = now + timedelta(minutes=180)
        return cutoff_time, UserWord.objects.none()
    cutoff_time = max(min_next_review + timedelta(minutes=180), now + timedelta(minutes=180))
