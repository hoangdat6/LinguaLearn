...
var react_component_name_exports = {};
__export(react_component_name_exports, {
  DEFAULT_EXCLUDE: () => DEFAULT_EXCLUDE,
  DEFAULT_INCLUDE: () => DEFAULT_INCLUDE,
  default: () => react_component_name_default,
  reactComponentNamePlugin: () => reactComponentNamePlugin,
  transform: () => transform
});
module.exports = __toCommonJS(react_component_name_exports);
...
var pathReferencesImport = (path, moduleSource, importName, asType, defaultNamespace = false) => {
  const identifier2 = unwrapPath(path, t.isIdentifier);
  if (identifier2) {
    ...
    if (binding && binding.kind === "module") {
      ...
      if (isPathValid(importParent, t.isImportDeclaration) && importParent.node.source.value === moduleSource) {
        if (isPathValid(importPath, t.isImportSpecifier)) {
          const key = t.isIdentifier(importPath.node.imported) ? importPath.node.imported.name : importPath.node.imported.value;
  ...
  const memberExpr = unwrapPath(path, t.isMemberExpression) || unwrapPath(path, t.isOptionalMemberExpression);
  if (memberExpr) {
...
var reactComponentNamePlugin = (0, import_unplugin.createUnplugin)(
  (options) => {
...
var react_component_name_default = reactComponentNamePlugin;
